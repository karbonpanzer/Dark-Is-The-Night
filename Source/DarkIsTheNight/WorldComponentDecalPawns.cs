...
namespace DarkIsTheNight
{
    public class WorldComponentDecalPawns : WorldComponent
    {
        ...
        {
            base.ExposeData();
            Scribe_Collections.Look(ref _pawns, "DarkIsTheNightDecalPawns", LookMode.Reference);
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                {
                    Log.Error("[DarkIsTheNight] Some decal pawns were null after loading.");
                }
                if (_pawns.RemoveWhere(x => x.def == null || x.kindDef == null) != 0)
                {
                    Log.Error("[DarkIsTheNight] Some decal pawns had null def after loading.");
                }
            }
        }
