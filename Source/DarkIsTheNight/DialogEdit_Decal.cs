...
namespace DarkIsTheNight
{
    ...
    public sealed class DialogEditDecals : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(titleRect, "DarkIsTheNight_StyleDecalsTitle".Translate(_pawn.Name.ToStringShort));
        ...
        private void DrawTabs(Rect rect)
        {
            ...
            {
                new TabRecord("DarkIsTheNight_Decals_Armor".Translate(), () => SetTab(DecalSlot.Armor), _curTab == DecalSlot.Armor),
                new TabRecord("DarkIsTheNight_Decals_Helmet".Translate(), () => SetTab(DecalSlot.Helmet), _curTab == DecalSlot.Helmet)
            };
            ...
            Widgets.CheckboxLabeled(new Rect(controlBar.x, controlBar.y, controlBar.width * 0.5f, controlBarH),
                "DarkIsTheNight_Decals_Enabled".Translate(), ref active);
            if (wasActive != active)
            {
            ...
            if (Widgets.ButtonText(new Rect(controlBar.xMax - 140f, controlBar.y, 140f, controlBarH),
                "DarkIsTheNight_Decals_RandomSymbol".Translate()) && _symbols.Count > 0)
            {
        ...
        private void DrawColors(Rect rect)
        {
            ...
            if (showIdeo)
            {
                if (Widgets.ButtonText(new Rect(btnX, curY, btnW, btnH), "DarkIsTheNight_Decals_IdeoColor".Translate()))
                {
            ...
            if (Widgets.ButtonText(new Rect(btnX, curY, btnW, btnH), "DarkIsTheNight_Decals_RandomColor".Translate()))
            {
            ...
            if (showFav)
            {
                if (Widgets.ButtonText(new Rect(btnX, curY, btnW, btnH), "DarkIsTheNight_Decals_FavColor".Translate()))
                {
                    color = favColor;
                    SoundDefOf.Tick_Low.PlayOneShotOnCamera();
                }
        ...
        {
            if (Widgets.ButtonText(new Rect(inRect.x, inRect.yMax - ButSize.y, ButSize.x, ButSize.y), "DarkIsTheNight_Decals_Cancel".Translate()))
            {
                _committed = false;
                Close();
            }

            if (Widgets.ButtonText(new Rect(inRect.xMin + inRect.width / 2f - ButSize.x / 2f, inRect.yMax - ButSize.y, ButSize.x, ButSize.y), "DarkIsTheNight_Decals_Reset".Translate()))
            {
            ...
            if (Widgets.ButtonText(new Rect(inRect.xMax - ButSize.x, inRect.yMax - ButSize.y, ButSize.x, ButSize.y), "DarkIsTheNight_Decals_Accept".Translate()))
            {
                _committed = true;
                Close();
            }
